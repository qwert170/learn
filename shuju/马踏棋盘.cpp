...
void print_stack(){
    ...
    for(int i = 0;i < MAX_STEPS;i++){
        printf("x:%d  y:%d direction = %d\n",Adr_Stack[i].x_adr,Adr_Stack[i].y_adr,Adr_Stack[i].direction);
    }
    printf("\n\n");
}
...
int t = 1;
void print_steps(){
    printf("(%d,%d)",Adr_Stack[top].x_adr,Adr_Stack[top].y_adr);
    t++;
    if(t == ROW){
        printf("\n");
        t = 0;
...
void run_horse_tanxin(){
    ...
    while(1){
        ...
        for(int i = 0;i < ROW;i++){
            ...
            if((x_next > 0 && x_next <= COL) && (y_next > 0 && y_next <= ROW) && chess[x_next][y_next] == 0 ){
                //对下一步的下一步判断是否可以走
                for(int j = 0;j < ROW;j++){
                    ...
                    //可以走,next 对应下标+1
                    if((x_next_next > 0 && x_next_next <= COL) && (y_next_next > 0 && y_next_next <= ROW) && chess[x_next_next][y_next_next] == 0){
                        next[i]++;
                    }
                }
            }
        ...
        for(dir_now = Adr_Stack[top].direction + 1;dir_now < ROW;dir_now++){
            ...
            if((x_real <= COL && x_real > 0) && (y_real <= ROW && y_real > 0) && chess[x_real][y_real] == 0){
                //入栈
        ...
        if(Adr_Stack[top].direction >= 7){
            printf("\n out:(%d,%d) \n",Adr_Stack[top].x_adr,Adr_Stack[top].y_adr);
            chess[Adr_Stack[top].x_adr][Adr_Stack[top].y_adr] = 0;
            //记录出栈次数
            out_stack++;
            pop_stack();
        }
